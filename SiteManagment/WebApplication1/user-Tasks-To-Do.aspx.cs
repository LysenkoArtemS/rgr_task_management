      ...
      /*  protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            {
                List<TaskInfo> tasks = new List<TaskInfo>();
                string connectionString = ConfigurationManager.AppSettings["connectinDB"];
                using (MySqlConnection connection = new MySqlConnection(connectionString))
                {
                    connection.Open();
                    using (MySqlCommand command = new MySqlCommand("SELECT * FROM tasks WHERE parentId IS NULL and actingID = @actingID" , connection))
                    {
                        command.Parameters.AddWithValue("@actingID", Session["id"]);
                        using (MySqlDataReader reader = command.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                    ...
                    foreach (TaskInfo task in tasks)
                    {
                        FillTaskDetailsFromDatabase(task, connectionString);
                        FillSubtasksFromDatabase(task, connectionString);
                    }
                    Tasks = tasks;
